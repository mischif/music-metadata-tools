...
""" 
    id3autosort: Sort MP3 files based on ID3 tags
    Jeremy Brown, http://www.mischivous.com/code/id3autosort/
    ID3 reader by Ned Batchelder, http://nedbatchelder.com/code/modules/id3reader.html
"""

from id3autosort import __version__

import glob, os, struct, sys, zlib, shutil
...
def alsort(strStartDir,strEndDir):
	if os.sep not in strStartDir[-1]:
		strStartDir += os.sep
	if os.sep not in strEndDir[-1]:
		strEndDir += os.sep
	listMusic = glob.glob(strStartDir+'*.mp3')
	for mp3 in listMusic:
		id3 = Reader(mp3)
		strArtist = str(id3.getValue('performer'))
		strAlbum = str(id3.getValue('album'))
		if strAlbum == 'None' or strArtist == 'None':
			continue
		if not os.path.isdir(strEndDir+strArtist+os.sep+strAlbum):
			os.makedirs(strEndDir+strArtist+os.sep+strAlbum)
		else:
			pass
		shutil.move(mp3,strEndDir+strArtist+os.sep+strAlbum)

def asort(strStartDir,strEndDir):
	if os.sep not in strStartDir[-1]:
		strStartDir += os.sep
	if os.sep not in strEndDir[-1]:
		strEndDir += os.sep
	...
	for mp3 in listMusic:
		...
		if strArtist == 'None':
			continue
		if not os.path.isdir(strEndDir+strArtist):
			os.makedirs(strEndDir+strArtist)
		else:
			pass
		shutil.move(mp3,strEndDir+strArtist)

def lsort(strStartDir,strEndDir):
	if os.sep not in strStartDir[-1]:
		strStartDir += os.sep
	if os.sep not in strEndDir[-1]:
		strEndDir += os.sep
	...
	for mp3 in listMusic:
		...
		if strAlbum == 'None':
			continue
		if not os.path.isdir(strEndDir+strAlbum):
			os.makedirs(strEndDir+strAlbum)
		else:
			pass
		shutil.move(mp3,strEndDir+strAlbum)

def rsort(strStartDir,strEndDir):
	if os.sep not in strStartDir[-1]:
		strStartDir += os.sep
	if os.sep not in strEndDir[-1]:
		strEndDir += os.sep
	...
	for mp3 in listMusic:
		...
		if strAlbum == 'None' or strArtist == 'None':
			continue
		if not os.path.isdir(strEndDir+strArtist+' - '+strAlbum):
			os.makedirs(strEndDir+strArtist+' - '+strAlbum)
		else:
			pass
		shutil.move(mp3,strEndDir+strArtist+' - '+strAlbum)

def tsort(strStartDir,strEndDir):
	if os.sep not in strStartDir[-1]:
		strStartDir += os.sep
	if os.sep not in strEndDir[-1]:
		strEndDir += os.sep
	...
	for mp3 in listMusic:
		...
		if strAlbum == 'None' or strArtist == 'None':
			continue
		if not os.path.isdir(strEndDir+strArtist+os.sep+strArtist+' - '+strAlbum):
			os.makedirs(strEndDir+strArtist+os.sep+strArtist+' - '+strAlbum)
		else:
			pass
		shutil.move(mp3,strEndDir+strArtist+os.sep+strArtist+' - '+strAlbum)

def halp():
	print 'Usage: id3autosort.py [OPTION] source_mp3_folder destination_mp3_folder\n'
	print 'OPTIONS:\n'
	print '(default)	Sort in Artist/Album structure'
	print '-a		Sort in Artist Only structure'
	print '-l		Sort in Album Only structure'
	print '-r		Sort in Artist - Album structure'
	print '-t		Sort in Artist/Artist - Album structure'
	return
...
if __name__ == '__main__':
	if '-?' in sys.argv or '--help' in sys.argv:
		...
		halp()
	elif len(sys.argv) == 3 and os.path.isdir(sys.argv[1]) == True:
		alsort(sys.argv[1],sys.argv[2])
	elif len(sys.argv) == 3 and os.path.isdir(sys.argv[1]) == False:
		halp()
	elif len(sys.argv) == 4 and os.path.isdir(sys.argv[2]) == True:
		if sys.argv[1] == '-a':
			asort(sys.argv[2],sys.argv[3])
		elif sys.argv[1] == '-l':
			lsort(sys.argv[2],sys.argv[3])
		elif sys.argv[1] == '-r':
			rsort(sys.argv[2],sys.argv[3])
		elif sys.argv[1] == '-t':
			tsort(sys.argv[2],sys.argv[3])
		else:
			pass
	elif len(sys.argv) == 4 and os.path.isdir(sys.argv[2]) == False:
		halp()
	else:
		halp()
